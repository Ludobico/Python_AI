...
def firstmenu():
    ...
    url = 'https://movie.naver.com/movie/sdb/rank/rmovie.naver?sel=cnt&date={0}'.format(inputdate)
    response = READYtoCRAWLING.urlopen(url)
    soup = BeautifulSoup(response, 'html.parser')
    i = 1
    f1 = open('naverMOVIE 조회순{0}.csv'.format(inputdate), 'w', encoding='utf8', newline='')
...
def secondmenu():
    inputdate = input('날짜를 선택해라: ')
    url = 'https://movie.naver.com/movie/sdb/rank/rmovie.naver?sel=cur&date={0}'.format(inputdate)
    response = READYtoCRAWLING.urlopen(url)
    soup = BeautifulSoup(response, 'html.parser')
    i = 1
    r = 0
    f2 = open('naverMOVIE 평점순(현재상영영화){0}.csv'.format(inputdate), 'wt', encoding='utf8', newline='')
...
def thirdmenu():
    ...
    if(firstpage == 1):
        ...
        i = ((firstpage - 1) * 50) + 1
    r = 0
    f3 = open('naverMOVIE 평점순(모든영화){0}.csv'.format(inputdate), 'wt', encoding='utf8', newline="")
    wr3 = csv.writer(f3, delimiter=",", quotechar="|")
    while(firstpage <= lastpage):
        url = 'https://movie.naver.com/movie/sdb/rank/rmovie.naver?sel=pnt&date={0}&page={1}'.format(inputdate,firstpage)
        response = READYtoCRAWLING.urlopen(url)
        soup = BeautifulSoup(response, 'html.parser')
        for data2 in soup.select('td.point'):
        ...
        for data in soup.select('div.tit5 a'):
            ...
            thirdlist = ['{0}등 {1} {2}'.format(i,data.string,thirdranklist[r])]
